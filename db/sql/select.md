...
ELSE 'C' END AS GRADE
, CASE WHEN SCORE >= 96 THEN A.SAL * 0.2
WHEN SCORE >= 90 THEN A.SAL * 0.15
WHEN SCORE >= 80 THEN A.SAL * 0.1
ELSE 0 END AS BONUS
FROM HR_EMPLOYEES A INNER JOIN 
(
SELECT B.EMP_NO
, AVG(COALESCE(B.SCORE, 0)) AS SCORE
FROM HR_GRADE B
GROUP BY B.EMP_NO
...
, T.EMP_NAME
, T.GRADE
, CASE WHEN T.GRADE = 'S' THEN T.SAL * 0.2
...
, A.FISH_TYPE
FROM (SELECT ID, FISH_TYPE, CASE WHEN COALESCE(LENGTH, 0) <= 10 THEN 10 ELSE LENGTH END AS LENGTH FROM FISH_INFO) AS A
...
SELECT A.QUARTER, COUNT(1) AS ECOLI_COUNT FROM
(SELECT CASE WHEN MONTH(DIFFERENTIATION_DATE) <= 3 THEN '1Q'
WHEN MONTH(DIFFERENTIATION_DATE) <= 6 THEN '2Q'
WHEN MONTH(DIFFERENTIATION_DATE) <= 9 THEN '3Q'
ELSE '4Q' END AS QUARTER FROM ECOLI_DATA) AS A
GROUP BY A.QUARTER
ORDER BY A.QUARTER
```
